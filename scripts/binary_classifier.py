...
# variables = ["pt_1", "pt_2","pt_3","pt_4","eta_1","eta_2","eta_3","eta_4",
...
if not (args.load and os.path.isfile("dataframes/{}_{}.pkl".format(args.channel,args.year))):
  ...
  # Add Test dataframe
  add_sig_df = Dataframe()
  add_sig_df.LoadRootFilesFromJson("json_selection/{}_{}_sig_test.json".format(args.channel,args.year),variables)
  add_sig_df.NormaliseWeights()
  add_sig_df.dataframe.loc[:,"y"] = 1
  for i in abs_variables: 
    add_sig_df.dataframe[i] = add_sig_df.dataframe[i].abs()
  print "Additional Samples Dataframe"
  print add_sig_df.dataframe.head()
  print "Length =",len(add_sig_df.dataframe)
  print "Weight Normalisation =",add_sig_df.dataframe.loc[:,"weights"].sum()
...
DrawVarDistribution(df_total,2,'q_4',-2,2,"bc_q_4_distribution")
...
add_test = test[(test.loc[:,"y"]==0)]
add_df_total = pd.concat([add_test,add_sig_df.dataframe],ignore_index=True, sort=False)
add_y_test = add_df_total.loc[:,"y"]
add_wt_test = add_df_total.loc[:,"weights"]
add_X_test = add_df_total.drop(["y","weights"],axis=1)
...
preds = probs[:,1]
DrawROCCurve(y_test,preds,wt_test,"roc_curve_original")


probs_ = xgb_model.predict_proba(add_X_test)
preds_ = probs_[:,1]
DrawROCCurve(add_y_test,preds_,add_wt_test,"roc_curve_Additional_Signal_Samples")
