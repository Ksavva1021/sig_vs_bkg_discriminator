...
from UserCode.sig_vs_bkg_discriminator.plotting import DrawClosurePlots, DrawReweightPlots
import argparse
...
if not args.load_dataframes:
  ...
  bkg_df = Dataframe()
  bkg_df.LoadRootFilesFromJson("json_selection/ff_4tau/ff_mc_{}.json".format(args.channel),fitting_variables+[y_multiclass],quiet=(args.verbosity<2))
  bkg_df.TrainTestSplit()
  category_values = [ind for ind,val in enumerate(args.channel) if val == "t"]
...
print multiclass_model.predict_proba(X_train_bkg_df)
print multiclass_model.predict_proba(X_test_bkg_df)
exit()
...
for ind, obj in enumerate(args.channel):
  if obj == "t":
    ...
    if not args.load_dataframes:
      ...
      fail_dr = Dataframe()
      fail_dr.LoadRootFilesFromJson("json_selection/ff_4tau/ff_data_{}.json".format(args.channel),fitting_variables,quiet=(args.verbosity<2),in_extra_name="dr_fail_{}".format(obj_num),replace=replace)
      ...
      if first_loop: # pass_df will be the same no matter what ratio is calculated
        pass_dr = Dataframe()
        pass_dr.LoadRootFilesFromJson("json_selection/ff_4tau/ff_data_{}.json".format(args.channel),fitting_variables,quiet=(args.verbosity<2),in_extra_name="dr_pass".format(obj_num),replace=replace)
        pass_dr.TrainTestSplit()
        if args.verbosity > 0: PrintDatasetSummary("Pass dr dataframe",pass_dr.dataframe)
    
      fail_dr_alt = Dataframe()
      fail_dr_alt.LoadRootFilesFromJson("json_selection/ff_4tau/ff_data_{}.json".format(args.channel),fitting_variables,quiet=(args.verbosity<2),in_extra_name="dr_alt_fail_{}".format(obj_num),replace=replace)
      ...
      pass_dr_alt.LoadRootFilesFromJson("json_selection/ff_4tau/ff_data_{}.json".format(args.channel),fitting_variables,quiet=(args.verbosity<2),in_extra_name="dr_alt_pass_{}".format(obj_num),replace=replace)
      ...
      fail_cr = Dataframe()
      fail_cr.LoadRootFilesFromJson("json_selection/ff_4tau/ff_data_{}.json".format(args.channel),fitting_variables,quiet=(args.verbosity<2),in_extra_name="cr_fail_{}".format(obj_num),replace=replace)
      fail_cr.TrainTestSplit()
      if args.verbosity > 0: PrintDatasetSummary("Fail cr {} dataframe".format(obj_num),fail_cr.dataframe)
      
      pass_cr = Dataframe()
      pass_cr.LoadRootFilesFromJson("json_selection/ff_4tau/ff_data_{}.json".format(args.channel),fitting_variables,quiet=(args.verbosity<2),in_extra_name="cr_pass_{}".format(obj_num),replace=replace)
